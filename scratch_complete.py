...
def cleanDf(df, userid, db):
    ...
    print("aahahhahahaha")
    print(userid)
    user = mycol.find_one(ObjectId("62bc13cbfb7f55527094fe23"))
    print("teeeest",user)
    ...
    if(user["gluten_free"]):
        print("okkkkk")
        mask = (df['tags'].str.contains('gluten-free', case=False, na=False))
        df = df[mask]
    if(user["vegetarien"]):
        mask = (df['tags'].str.contains('vegetarian', case=False, na=False))
        df = df[mask]
...
def like_to_rec(listLikes, sig, df):
    ...
    for i in listLikes:
        multiRec.append(give_rec(int(i), sig, df))
...
def recommendationClean(idUser):
    ...
    df = creationDf(listL,listD,iduser,db)
